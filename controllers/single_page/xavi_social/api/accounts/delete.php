...
final class Delete extends PageController
{
    public function view(): void
    {
        ...
        $id = (int) ($json['accountId'] ?? 0);
        $did = trim((string) ($json['did'] ?? ''));

        if ($id <= 0 && $did === '') {
            $this->sendJson([
                'error' => 'bad_request',
                'message' => 'Provide accountId or did.',
            ], 400);
        }
        ...
        try {
            ...
            if ($id > 0) {
                $db->executeStatement(
                    'DELETE FROM XaviSocialAtprotoAccounts WHERE id = ? AND userId = ?',
                    [$id, $userId]
                );
            } else {
                $db->executeStatement(
                    'DELETE FROM XaviSocialAtprotoAccounts WHERE did = ? AND userId = ?',
                    [$did, $userId]
                );
            }
        } catch (\Throwable $e) {
