...
final class Me extends PageController
{
    public function view(): void
    {
        [$authMethod, $user] = $this->authenticate();

        $isAuthed = $user !== null && $user->isRegistered();

        $this->sendJson([
            'authenticated' => $isAuthed,
            'authMethod' => $authMethod,
            'userId' => $isAuthed ? (int) $user->getUserID() : null,
            'userName' => $isAuthed ? (string) $user->getUserName() : null,
        ]);
    }
